...
(function(gmu, $, undefined) {
    ...
    gmu.define('Calendar', {
        options: {
            ...
            minDate: null,
            /**
             * @property {Date|String} [limitDate=null]  设置不可以选择的日期
             * @namespace options
             */
            limitDate: null, //add
            /**
        ...
        _option: function(key, val) {
            var opts = this._options,
                date, minDate, maxDate, limitDate; //add
            //如果是setter
            if (val !== undefined) {
                    ...
                    case 'limitDate':
                        if (opts[key] instanceof Array) {
                            for (var i = 0; i < opts[key].length; i++) {
                                opts[key][i] = val ? $.calendar.parseDate(val[i]) : null;
                            }
                        }else{
                            opts[key] = val ? $.calendar.parseDate(val) : null;
                        }
        ...
        _generateHTML: function() {
            ...
            html += '</thead></tr><tbody><tr class="ui-calendar-gap">' +
                '<td colspan="7" style="display:none">&#xa0;</td></tr>';
            ...
            for (i = 0; i < rows; i++) {
                html += '<tr>';
                for (j = 0; j < 7; j++) {
                    html += this._renderDay(j, printDate, firstDay, drawMonth, selectedDate, today, minDate, maxDate, limitDate);
        ...
        _renderHead: function(data, drawYear, drawMonth, minDate, maxDate) {
            ...
            if (data.yearChangeable) {
                ...
                var dd = new Date();
                for (i = 1940, max = dd.getFullYear(); i <= max; i++) {
        ...
        _renderDay: function(j, printDate, firstDay, drawMonth, selectedDate, today, minDate, maxDate, limitDate) {
            ...
            var choice = false;
            if (limitDate != null) {
                if (limitDate instanceof Array) {
                    for (var t = 0; t < limitDate.length; t++) {
                        if (limitDate[t].getTime() == printDate.getTime()) {
                            choice = true;
                            break;
                        }
                    }
                }else{
                    if (limitDate.getTime() == printDate.getTime()) {
                        choice = true;
                    }
                }
            }
            ...
            unSelectable = otherMonth || (minDate && printDate < minDate) || (maxDate && printDate > maxDate) || choice;
    ...
    //添加更直接的option修改接口
    $.each(['maxDate', 'minDate', 'date', 'selectedDate', 'limitDate'], function(i, name) {
    ...
    /*自己定义的函数*/
    function changeD(){
        var d=$('.ui-state-active').text(),
            m=$('.ui-state-active').parent().attr('data-month');
        $('#choiceStart').text(m+'月'+d+'日')

        $('.ui-calendar-calendar td a').live('click',function(){
            var m=$(this).parent().attr('data-month');
            var d=$(this).text();
            $('#choiceStart').text(m+'月'+d+'日')
        })
    }
